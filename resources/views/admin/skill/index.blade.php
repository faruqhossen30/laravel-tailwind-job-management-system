...
@section('breadcrumb')
    <div class="flex justify-between items-center">
        <x-breadcrumb pageone="skill" />
        <x-button.button-plus route="{{ route('skill.create') }}" title="Create skill" />
    </div>
@endsection
@section('content')
                                ...
                                @forelse ($skills as $skill)
                                    <tr>
                                        <td
                                            ...
                                            {{ $skills->firstItem() + $loop->index }}</td>
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                                            {{ $skill->name }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                            ...
                                            <x-table.crudactionbutton route="skill" :id="$skill->id" />
