...
@section('breadcrumb')
    <div class="flex justify-between items-center">
        <x-breadcrumb pageone="Education" />
        <x-button.button-plus route="{{ route('education.create') }}" title="Create education" />
    </div>
@endsection
@section('content')
                                ...
                                @forelse ($educations as $education)
                                    <tr>
                                        <td
                                            ...
                                            {{ $educations->firstItem() + $loop->index }}</td>
                                        <td
                                            class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                                            {{ $education->name }}
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                            <x-table.crudactionbutton route="education" :id="$education->id" />
                                        </td>
                                    </tr>
                                @empty
                                    <td colspan="3"
